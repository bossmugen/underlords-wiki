---
name: "WolfPhenix"

aliases:
  - "Wolf Phenix"
  - "Wolfie"

pronouns: "she/her"

roles:
  - "Caporegime"
  - "News Editor"
  - "Game Officer"
eraStart: 2020

summary: "WolfPhenix enters UL as one of the four Altissa friends who came with Mugen, then becomes increasingly visible through creative and operational contribution: Photo Submissions and planners in 2021, announcements and house shorthand in 2022, the nineteen-person anniversary artwork and shared-file collaboration in 2023, planner continuity in 2024, another explicitly `UL ENOUGH` redesign in 2025, and reunion-era familiarity in 2026."

summaryEvidence: "archive-confirmed"
publication: "public"

claims:
  - text: "Mugen founded / started Underlords; WolfPhenix was one of the four friends who came with her from Altissa, and together Mugen, Ritha, WolfPhenix, Suzimasu, and HyaLuna form UL's five-person founding circle."
    evidence: "user-confirmed"
    sources:
      - label: "Mugs-confirmed founding canon"
        href: "/history"
  - text: "WolfPhenix is a current Caporegime / Secretary in the corrected UL command roster."
    evidence: "user-confirmed"
    sources:
      - label: "Mugs-confirmed command chart"
  - text: "WolfPhenix holds the News Editor appointment and also carries Game Officer status."
    evidence: "user-confirmed"
    sources:
      - label: "Mugs-confirmed appointments and Game Officer roster"
  - text: "On June 24, 2021, WolfPhenix posted a Photo Submissions attachment captioned `Ansun uwu`; the post received seven `fatlaugh` reactions."
    evidence: "archive-confirmed"
    date: "2021-06-24"
    sources:
      - label: "Photo Submissions recovered export"
        href: "/channels/photo-submissions"
  - text: "On November 8, 2021, Wolf posted a Weekly Planner and asked people to say what needed adding or changing; in April 2024, when Anayss asked for event updates, Wolf found an older planner, updated it, and again asked for missing events or incorrect times."
    evidence: "archive-confirmed"
    date: "2021-11-08 – 2024-04-08"
    sources:
      - label: "Recovered PR archive"
        href: "/channels/pr"
  - text: "On June 8, 2023, while Sye was working through collision and stale-copy problems in the communal doodle poster, Wolf suggested putting a doodle on a separate layer so it could be moved; Sye then clarified that her larger concern was a newer copy dropping somebody else's doodle entirely."
    evidence: "archive-confirmed"
    date: "2023-06-08"
    sources:
      - label: "Club Media Gallery"
        href: "/channels/pr"
  - text: "On September 3, 2022, a Wall exchange used `Wolfie` as shared shorthand: Gabu wrote `wolfie is giant, gilli is ant`, Wolf answered with two `Fat_Wheeze` emojis, and Mugen later wrote that Dean saw purple and thought `purple = wolfie`."
    evidence: "archive-confirmed"
    date: "2022-09-03"
    sources:
      - label: "Wall of Shame/Fame recovered export"
        href: "/channels/wall"
  - text: "On September 11 and September 22, 2022, WolfPhenix posted practical Announcements alerts that Star of Cassell / TAR LF Cassell was up."
    evidence: "archive-confirmed"
    date: "2022-09-11 – 2022-09-22"
    sources:
      - label: "Announcements recovered export"
        href: "/channels/announcements"
  - text: "On March 14, 2023, WolfPhenix posted `UL_Mafia_Ani.jpg` in the Louvre with `Happy Anniversary Underlords! Welcome to Our Friendly Neighborhood Mafia Fam!`. The room then played a character-identification game across the artwork; Mugen concluded `officers and snipers all in!` and `thats 19 people she drew`."
    evidence: "archive-confirmed"
    date: "2023-03-14 – 2023-03-15"
    sources:
      - label: "🎨 Louvre recovered export"
        href: "/channels/louvre"
  - text: "The recovered `UL_Mafia_Ani` media file is an exact attachment join to WolfPhenix's March 14, 2023 Louvre post. The validated recovered file is 7,763,429 bytes with SHA-256 `1aabe4bac7828d25caaefb2ed99c60e48873074963891093c210b17f13aed988`."
    evidence: "archive-confirmed"
    date: "2023-03-14"
    sources:
      - label: "🎨 Louvre media recovery"
        href: "/archive/louvre"
  - text: "On March 19, 2023, Kaede posted a long Louvre response saying WolfPhenix's anniversary drawing made him feel relief and happiness, reminded him that he had a family he loved and believed loved him, and made him grateful that Wolf had made a space for him among everyone."
    evidence: "archive-confirmed"
    date: "2023-03-19"
    sources:
      - label: "🎨 Louvre · Kaede response"
        href: "/channels/louvre"
  - text: "WolfPhenix answered Kaede that she was grateful to have him and everyone in their family, appreciated their time together, hoped for more `amazingly chaotic nonsensical memories`, and thanked him for being part of the family."
    evidence: "archive-confirmed"
    date: "2023-03-19"
    sources:
      - label: "🎨 Louvre · Wolfie reply"
        href: "/channels/louvre"
  - text: "On May 22, 2025, WolfPhenix returned to the Louvre with a three-stage UL doodle and explained that her first image `didnt feel UL ENOUGH`, so she redesigned it, showed the revised look and process, and ended with `Love you guys`."
    evidence: "archive-confirmed"
    date: "2025-05-22"
    sources:
      - label: "🎨 Louvre · UL doodle process"
        href: "/channels/louvre"
  - text: "On March 15, 2023, Mugen announced to Staff that WolfPhenix had `JUST DROPPED HER UL ANNIVERSARY PRESENT!` and directed everyone to the linked post in the Louvre."
    evidence: "archive-confirmed"
    date: "2023-03-15"
    sources:
      - label: "Announcements recovered export"
        href: "/channels/announcements"
  - text: "On August 14, 2026, WolfPhenix called reunion Whiskey `de corpses are aliiive` and said `time to draag`; about fifteen minutes later, when Mugen tagged WolfPhenix, Gilli, and Drink water reminder with `pls babysit halp` before leaving for a meeting, Wolf's surviving reply was `🫡`."
    evidence: "archive-confirmed"
    date: "2026-08-14"
    sources:
      - label: "Aug. 14 Whiskey living receipt"

antiFanon:
  - "Do not flatten UL's origin into five equal co-founders. Mugen started / founded Underlords; WolfPhenix came with her from Altissa and belongs to the five-person founding circle."
  - "Export-time role arrays do not establish when WolfPhenix became Caporegime, News Editor, or Game Officer."
  - "`purple = Wolfie` is archive-confirmed peer shorthand, not proof that WolfPhenix intentionally maintained a purple personal brand."
  - "Gabu's `wolfie is giant, gilli is ant` line is peer joking in a specific scene, not a real-world height claim."
  - "WolfPhenix posting an attachment in Photo Submissions proves uploader credit, not creator, photographer, or subject identity without separate evidence."
  - "The Louvre transcript plus exact attachment recovery now directly grounds WolfPhenix as the uploader/maker credited by the room for the 2023 anniversary artwork; the stylized character designs inside it are not real-world appearance evidence."
  - "Kaede's long response and Wolf's reply establish a warm, directly expressed family relationship in this scene. Do not inflate one anniversary exchange into an exclusive friendship ranking or unsupported pair biography."
  - "The June 2023 Sye/Wolf doodle exchange is one direct collaboration scene, not proof of a standing creative duo, friendship rank, or maker credit for the final communal poster."
  - "Mugen's 2026 `pls babysit halp` tag is a household/trust joke, not a new appointment, a seniority ranking among Wolf/Gilli/Gabu, or proof that the three were a standing historical management unit."
  - "Reaction counts are reception evidence, not friendship or importance rankings."

relatedPeople:
  - "mugen"
  - "ritha"
  - "suzimasu"
  - "hyaluna"
  - "gabu"
  - "gilli"
  - "ansun"
  - "anayss"
  - "ren"
  - "sye"
  - "kaede"

tags:
  - "founding-circle"
  - "caporegime"
  - "news-editor"
  - "game-officer"
  - "operations"
  - "wall"
  - "photo-submissions"
  - "pr"
  - "louvre"
  - "artist"
  - "anniversary"
  - "family"
---

# WolfPhenix

WolfPhenix belongs to UL's founding circle as one of the four Altissa friends who came with Mugen when Mugen founded the club. She is now a Caporegime, News Editor, and Game Officer, but the archive does not freeze her in March 2020 or turn the later titles into an appointment chronology. It shows her continuing to make and maintain things as UL changes around her.

## 2021 — CREATIVE CONTRIBUTION AND PRACTICAL PLANNING ARE ALREADY SIDE BY SIDE

In June 2021, Wolf posts Photo Submissions material captioned **`Ansun uwu`**. By November she is also posting a Weekly Planner and asking members to tell her what needs adding or changing.

Those are different kinds of contribution—one visual/social, one operational—but the pairing becomes characteristic over time. Wolf's archive is not only "artist" and not only "staff who posts schedules." She moves comfortably between making the group look like itself and making sure the group's information is usable.

## 2022 — THE ROOM HAS SHORTHAND FOR HER, AND SHE IS STILL DOING THE PRACTICAL WORK

By September 2022, **Wolfie** is shared shorthand. Gabu jokes **`wolfie is giant, gilli is ant`**; Wolf wheezes. Mugen later explains that Dean sees purple and thinks **`purple = wolfie`**.

Those are peer jokes, not verified height or proof of a deliberate purple personal brand. Their value is familiarity: Wolf has been around long enough for the room to compress her into associations everybody understands.

The same month she posts practical Star of Cassell alerts in Announcements. The social shorthand and the operational contribution coexist; one does not cancel the other.

## 2023 — THE ANNIVERSARY ART TURNS YEARS OF HOUSE LANGUAGE INTO A FAMILY PORTRAIT

On March 14, Wolf posts **`UL_Mafia_Ani.jpg`** in the Louvre with **`Happy Anniversary Underlords! Welcome to Our Friendly Neighborhood Mafia Fam!`**. The room starts identifying the characters. Mugen eventually concludes that the Officers and Snipers are all in: **nineteen people**.

The exact recovered image is joined to Wolf's post, so this is one of the places where maker/uploader credit is unusually solid. More importantly, the archive does not need an outside critic to guess what the picture meant to the people inside it.

Five days later, Kaede writes eight messages about the drawing. He says seeing himself among people he considers wonderful gave him relief and happiness, reminded him he had a family he loved and believed loved him, and made him grateful Wolf had **made a space for him among everyone**. Wolf answers with gratitude, family language, and a hope for more **`amazingly chaotic nonsensical memories`**.

That exchange is not an exclusive friendship ranking. It is direct evidence of one member telling the artist what inclusion in her communal picture did to him.

Later that June, Wolf is in the communal doodle project helping Sye think through layer movement while Sye diagnoses a larger stale-copy problem. Again, visual work and practical file behavior occupy the same person.

## 2024–2025 — THE CONTRIBUTION DOES NOT END WITH THE ANNIVERSARY CENTERPIECE

In April 2024, Anayss asks for event updates. Wolf finds an older planner, revises it, and again asks for missing events or incorrect times. The 2021 planner was not a one-season artifact; she is still willing to reopen the file years later and make it useful again.

In May 2025, she returns to the Louvre with a three-stage doodle process and gives the design test herself: the first idea **`didnt feel UL ENOUGH`**. So she redesigns it, shows the revised look and process, and ends with **`Love you guys`**.

That sentence is a better description of mature UL visual culture than a fixed brand guide. The question is not only whether the object uses the right logo. It is whether Wolf feels it has accumulated enough of the group's own language to read as them.

## 2026 — FOUNDING HISTORY HAS BECOME REUNION FAMILIARITY

When Whiskey wakes up during the August reunion, Wolf calls it **`de corpses are aliiive`** and says **`time to draag`**. A little later Mugen tags Wolf, Gilli, and Gabu with **`pls babysit halp`** before leaving for a meeting; Wolf replies `🫡`.

That is not a new appointment or hidden seniority chart. It is the late-era version of the same continuity: Wolf is still one of the people Mugen can address without reintroducing the relationship, and Wolf is still participating in the room as both contributor and family member.

The biography therefore progresses cleanly: pre-UL friend / founding-circle member → visible creative and planning contributor → socially recognizable house figure → artist of a major communal memory object → continued maintenance and redesign years later → reunion-era familiarity. The titles matter. The years explain the person wearing them.
