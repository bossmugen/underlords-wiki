...
export const early0836IntegratedEpisodes: Episode[] = [
  {
    id: "ghoulie-pandas-house-of-shame",
    title: "Ghoulie & Panda's House of Shame",
    season: "2021",
    date: "July 7, 2021",
    kind: "Wall live-fire exercise / the evidence room briefly acquires tenants",
    logline: "Ren opens the day by announcing `Ghoulie back to claim her wall`. A few hours later Baby Lyssa/Ghoulie tells Marian/Panda she is `lackin`; Marian insists `i'm behaving`, then immediately confesses that she has said plenty of Wall-worthy things and complains that Eos is `too vanilla to shame me`. Eos answers that challenge twenty-one minutes later by POSTING a receipt, says they are `Debating on sending more`, and later admits to holding another candidate that might get them `divorced`. The interesting part is the hesitation: Eos is plainly not empty-handed, just doing consequence math before filing. Gilli encourages the prosecution. Tofu POSTS another image; Baby Lyssa answers `GOD DAMNIT YOU TOO??` and `i’m no longer safe`, making her the probable implicated subject without turning reaction timing into pixel identification. Marian offers solidarity. Ghoulie replies `this is my house y’all just livin in it`, and the two immediately start fake-bickering about imaginary dishes, kitchen duty and whose food was on the plates. Mugen eventually points out `Yall can just talk here and we won't even need ss`: the Wall has crossed the line where live conversation itself is now admissible evidence. Snow supplies the closing paperwork by mock-renaming the room `Ghoulie & Panda's house of shame` and suggesting they may need their own subchannel because there is so much content. The house/kitchen material is plainly the joke, not evidence that Ghoulie and Panda literally lived together; Eos's `divorced` line does not establish a spouse or partner; and the Ren, Eos and Tofu images remain visually uninspected, so POSTED BY stays separate from MADE BY, CAPTURED BY and FEATURING.",
    cast: ["Baby Lyssa", "Marian Kage", "Eos", "Tofu", "Mugen", "Snow", "Gilli", "Ren"]
  },
  {
    id: "shiyax-tree-branch-has-an-address",
    title: "Shiya's Tree Branch Has a Channel Address",
    season: "2021",
    date: "July 1, 2021",
    kind: "Slice continuity / ordinary-life lore acquires filing instructions",
    logline: "Baby Lyssa arrives in the Wall with an extremely reasonable question: `but can you explain the tree branch drowning ???`. ShiyaX does not explain. He answers `Tis in slice` and leaves Mugen to supply the missing sentence: `Yaya's making a bedside aquarium for his pet tree branch`. The underlying Slice artifact still needs an exact join, so the wiki does not describe pixels or pretend to know what the setup looked like. The social mechanism is already clear without them. By mid-2021 one of Shiya's real-world projects has become recognizable enough outside Slice that another room can ask for the backstory, and Shiya can answer with a room address because the earlier episode is apparently already filed there. That sits beautifully beside his work captions from Slice — `wedding shyt`, `Very smol wedding haps`, then later setup/progress/finish/teardown updates — because Shiya tends to let ordinary physical-world projects arrive while they are still happening instead of waiting for them to become polished portfolio objects. It also makes a later contradiction funnier: the same man who tells people `Take it to #🥃💬｜whiskey-room guys` when a conversation belongs elsewhere answers HamitteY's `u missed the chaos` with `I AM THE CHAOS`. Room literacy and self-declared disorder are not competing biographies here. Shiya can know exactly where the paperwork goes and still consider himself part of the hazard supply.",
    cast: ["ShiyaX", "Baby Lyssa", "Mugen", "HamitteY"]
  },
  {
    id: "pineapple-buttplug-screenshot-memory",
    title: "The Screenshot Isn't Even Here Yet",
    season: "2021",
    date: "May 11, 2021",
    kind: "Wall evidence economy / the external evidence locker is apparently everybody's camera roll",
    logline: "Korea enters the Wall with no attachment and one threat: `wait til i find the pineapple buttplug ss :v_sip:`. The file is not there. Thirty-four seconds later Mugen answers `Owo my favee`. It is not a structured reply, but the timing plus the room's next direct-reply recognition makes the local referent probable: the screenshot/event has apparently been remembered well enough that Mugen is already ranking it before anybody recovers the file. Ren TRUE-REPLIES to Korea's exact message, keyboard-smashes, then says `I REMEMBER THAT`; Queen Shimoko TRUE-REPLIES to the same parent with two spit emotes. That is the whole cursed mechanism: by May 2021 a Wall receipt could remain socially alive before anybody had even recovered the file. Somebody remembered there was a screenshot somewhere, Mugen apparently had it on a favorites list in her head, and the public evidence room could resume proceedings on shared memory plus the promise of eventual excavation. The surviving sequence does not prove Korea later found or posted the screenshot, what it showed, who appeared in it, or who made or captured it; `my favee` is evidence of likely receipt preference/recognition, not anybody's sexual preference. The screenshot is absent. The rating, incredibly, is already in.",
  ...
  {
    id: "three-firelegacy-visitors-at-the-door",
    title: "FireLegacy Sends Three Ranks Through the Front Door",
    season: "2020",
    date: "November 26–27, 2020",
  ...
  },
  {
    ...
    logline: "Zhēnxī first enters this little Wall arc on the wrong side of the bench. Ren POSTS `image0.png`, later tags Zhēnxī, and six seconds after the tag Zhēnxī arrives with `NOOOOOOOOOOO`; sixteen seconds later comes the more specific panic, `NOT THE WALL OF SHAME FAME`. Sou later describes the speed as `shamed within 6 minutes of entry`; Ren wonders whether it is a new record. The exact offense is not safe to name from the uninspected filing. A year later the posture has completely changed. Zhēnxī POSTS `Screenshot_20220707-232429_Discord.jpg`, waits ten seconds, tags Ren with `so uh-`, and Ren TRUE-REPLIES to the screenshot with a skull, then `STAWP`, then the fatal aggravating detail: `This happened like 4 months ago so that’s even worst`. Zhēnxī laughs and rules simply: `Deserving`. Ren's `like 4 months ago` stays approximate, and Zhēnxī gets POSTED BY only because the screenshot pixels/provenance are unresolved. The good human read needs no extra machinery: being prosecuted did not make Zhēnxī anti-Wall. A year later Zhēnxī is storing an old Discord receipt long enough to drag Ren back into court and reject the statute of limitations. The defendant learned procedure and immediately became the prosecutor.",
    cast: ["Zhēnxī", "Ren", "Sou"]
  },
  {
    id: "rotini-lurks-then-gets-caught",
    title: "Rotini Goes From Lurking WC to `SHH EVERYTHING'S FINE`",
    season: "2022–2024",
    date: "January 24, 2022 → January 3, 2024",
    kind: "Wall character texture / the receipt-bringer eventually enters the crime scene",
    logline: "In January 2022 RotiniLM is doing exactly what the Wall rewards: `Just lurking the WC:R_stare:` while POSTING `20220124_155327.jpg`, then later returning with `One more-` and another attachment. The first line explicitly places the source in World Chat; the sealed images stay POSTED BY Rotini only, with their deeper provenance and subjects unresolved. Two years later Daya POSTS a screenshot with `walked into a crime being committed`, then separately tags RotiniLM and Snow four seconds later. Rotini appears in that immediate tagged scene with the least reassuring possible reassurance: `SHH EVERYTHING'S FINE`, followed by a heart. Rotini's message is Reply-type and mentions Daya, but the surviving handoff does not expose the reference message ID, so the wiki does not pretend it has an exact screenshot-parent join. The narrow reversal is already funny enough. In 2022 Rotini is the person quietly watching World Chat and bringing receipts home; in 2024 Daya walks into some unresolved mess, summons Rotini and Snow, and Rotini's first visible instinct is containment. That does not make Rotini a permanent spy, criminal, or evidence officer. It gives one perfect little Wall beat where the observer has apparently become somebody inside the evidence and immediately asks the witness to stop looking directly at it.",
    cast: ["RotiniLM", "Daya", "Snow"]
  }
];