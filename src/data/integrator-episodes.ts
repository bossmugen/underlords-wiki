...
export const integratedEpisodes: Episode[] = [
  ...
  },
  {
    ...
    logline: "On July 25, 2022 Anayss announces a Land of Conquest outfit mechanic as `something new brought to you by @Yelik`: choose an animal, food, or item profile picture, then `Match it with your outfit!`, with `Yeye’s pic` supplied as inspiration. A week later Anayss names the returning mechanic `MATCH YOUR PFP DAY!!!`. The idea stops belonging to one announcement: Wolf Phenix carries the reminder on August 16, Life carries another in October, and by January 2023 Anayss is calling it `our famous MATCH YOUR PFP DAY!!!!`; later posts explicitly set it aside for substitute themes and then bring it back. The safe genealogy is therefore unusually clean without pretending to prove an absolute coining moment: Yelik is contemporaneously credited with the new suggestion → Anayss operationalizes and names the recurring day → other people carry it → the house calls it famous. Anayss's birth-announcement reference images and Yelik's next-day image attachments remain visually uninspected, so POSTED BY is not collapsed into MADE BY, CAPTURED BY, or FEATURING.",
  ...
  },
  {
    ...
    season: "2021",
    date: "January 3, 2021",
    kind: "One-day administrative decree",
    logline: "Tae announces `new rule` → `Always vote out gabu`, then later addresses Gabu with `Gabu` → `Go to jail`. The same-day callback is real; a targeted recurrence check found no trustworthy later life for the exact rule, so this remains one afternoon of fake legislation rather than standing House Law.",
    cast: ["Tae", "Gabu"]
  },
  {
