...
export const run43NarrativeCharacterBios: Record<string, CharacterNarrative> = {
  ...
  mugen: beforeSection(run41NarrativeCharacterBios.mugen, "Overall character", {
    ...
    paragraphs: [
      ...
      "The same game also says something more interesting about why she likes building little systems in the first place. Mugen states the point outright: WHOMSTF is about `how well we know our fellow staff and VIPs`, and if somebody does not know them yet, then it is `the time to get to know them`. The first correct guesser becomes the next `confused dumdum`; Mugen herself plays as an ordinary guesser; later she tells everybody they can restart the chain without waiting for her, and Ren does exactly that the next day. The structure is doing social work while making its own author dispensable. That does not prove a universal decentralization doctrine. It does show Mugs designing one game so attention, knowledge and temporary control keep circulating after Boss has stopped holding the clipboard.",
