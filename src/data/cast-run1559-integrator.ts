...
/** Run 1559 integrator: Cele future-reader seed + Ricochet callback relay. */
...
function applyRicochet(characters: Character[]): void {
  ...
  characters[index] = {
    ...
    claims: unique([
      ...
      "A July 2021 Photo Submissions post by Panda / Marian captioned `with my only @Eos` gives the Eos–Panda relationship a warm shared-presentation mode outside Wall receipt sparring; the attachment is POSTED BY Panda only unless separate media inspection resolves creator/capturer/subjects.",
    ]),
    antiFanon: unique([
      ...(previous.antiFanon ?? []),
      ...
      "Panda's `with my only @Eos` is warm public shared-presentation language, not proof of exclusivity or relationship status.",
    ]),
  };
}
