...
// Anayss — fold the Daycare tail into the existing person instead of leaving her as a Wall-only receipt hunter.
{
  ...
  allCharacters[index] = {
    ...
    era: "2020–present",
    logline:
      "The person who once admitted she gets lost keeps becoming one of the people who makes UL easier to navigate: welcomes, practical game help, battle clocks, bad dad jokes she apologizes for in advance, and a 2023 anniversary message that makes clear the room stopped being a quick visit a long time ago. Wall still catches her playing prosecutor and defendant, but usefulness and belonging are the bigger story.",
    ...
    claims: appendUnique(anayss.claims, [
      "Stable account 698943572875149342 ties the 2021–2023 Daycare material to the existing canonical Anayss owner.",
...
// Joy — a small Wall owner built only from what the authored/reply structure can actually carry.
{
  ...
  const next: ExtendedCharacter = {
    ...joy,
    logline:
      "Low-volume Wall participant with a dry little habit of dropping a line and letting everybody else build the courtroom around it. In 2020 Ren and BoobooHoney can pick up Joy's fragments and turn them toward Fame/Wall procedure; in 2024 the same account is still capable of returning with the compact retrospective `…we laughed about it`.",
    tags: appendUnique(joy.tags, ["Archive cast", "Wall", "Low-volume", "Dry humor", "Receipt culture", "Petty Crimes"]),
    relationships,
    ...
    claims: appendUnique(joy.claims, [
      "Stable account 692166440417099868 anchors sixteen surviving authored Wall messages across the 2020 material and a later 2024 return.",
