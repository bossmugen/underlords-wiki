# UNDERLORDS WIKI — Run 1242 Integration Close

MAIN's `archive-intake/INTEGRATOR_BRANCH_STATE.json` remains the controlling miner-consumption ledger. This close records the WIKI-side reconciliation for the same reviewed evidence frontier.

## Census and reviewed tails
- **51 tracked miner identities = 30 active refs + 21 historical/missing refs**.
- **0 newly discovered**, **0 closing pending review**, **0 backlog**, **0 overdue**.
- Both Club-Only recovery lanes remain current.
- **8** reviewed MAIN families remain publication/resynthesis debt; they are not unread miner intake.

Two tails were reviewed this run:

1. **Wall** `86d6827ca562687f3ec41e6ef1c6657220d4fed3 -> 73ad4f51051b507f3d572ae07ad61166d5cabe33`.
2. **Louvre / AI Art / Athenaeum / Other Games** `c22a21064d37b85873b21b93474fe7a73a133c94 -> a9ef45deccfcaa1572522b67ee3230270382e495`, caught by the closing recensus and reviewed under anti-starvation.

## Vanness — dedupe, do not staple on another receipt paragraph
The Wall suffix's strongest person synthesis is Vanness as **evidence pointer / mock-defendant / willing self-prosecutor when the disaster is unmistakably his own**. The new handoff centers `Im putting myself in wall of shame ... Shame me`, his failed-merge postmortem, `my luck is💩`, and the fact that he stays in the scene while Gilli, Candy, and Ren answer the self-file.

WIKI already owns this exact cumulative read in canonical `src/data/integrator-cast-run760-wall-vanness.ts`, including the same self-file, the same merge-loss sequence, and the same contradiction. The current dossier is stronger than the new handoff because it already combines that material with the earlier Mugen pointer and ShiyaX mock-defense lane.

**Result:** no duplicate public Vanness paragraph, no new Cast owner, no new Episode. The suffix is duplicate/corroboration and is safely consumable.

The same Wall review keeps q22 Erys provenance backstage: the Lobby welcome is now tied by true Reply geometry to Erys's self-intro, but there is still no native/system join timestamp. Feli and KARIT samples do not materially deepen their public owners. Visual material stays attribution-bounded.

## braindead — scope repair, not public characterization
The late Louvre tail does not supply a new WIKI character scene. It repairs an upstream source-boundary error: a plausible braindead person synthesis had been built from true announcements rows and then treated as if they were Louvre/Athenaeum character evidence.

That route is now quarantined inside the miner. The correction matters precisely because the synthesis sounded human and coherent: true scenes can still make a false corpus-specific biography when they are joined to the wrong room family. The assigned creative-room body therefore returns to direct-first/open rather than inheriting `SOWWY`/check-in, Gilli reassurance, dress-opinion, or beach material from announcements.

No public WIKI braindead owner using that invalid Louvre-owned synthesis was found, so there is nothing reader-facing to delete or rewrite.

## Public result
- WIKI reader-facing files changed: **0**.
- MAIN reader-facing files changed: **0** from these two tails.
- WIKI Astro / Pages build and deploy: **not run / not claimed**, because reader bytes did not change.
- MAIN source verification / Astro / built-output / deploy: **not run / not claimed**, because reader bytes did not change.

The reviewed Wall and Louvre frontiers may advance in MAIN's controlling ledger. Older MAIN person-first resynthesis debt remains separate from these no-reader-change reviews.
