...
The eight `pending_publication` families remain Backroom; Birthdays/PR/VC; Daycare; Louvre/AI/Athenaeum/Other Games; Meems; Whiskey; QOTD/Officers; Wall. This is reviewed MAIN publication debt, not unread miner work.
...
Athena's surviving Wall day is compact but coherent. She narrates a bad manhwa like contamination — lost `beauty sleep`, needs another comic to `wash my brain` — then accepts Mugen's redirect out of the Wall discussion with a single pat emote. WIKI already had Athena and an incident owner for the scene, so Run 644 deepened the existing person file rather than creating another Athena: maximal media reaction, almost zero social friction about being redirected, plus bounded Mugen relationship texture. Her two PNGs remain POSTED BY Athena only; pixels were not inspected.

Shiki q18 advanced backstage to `0+/35−` across 35 trustworthy-audited parents with 18/53 unaudited; it remains OPEN / PARTIAL. Woohyuk's authored-parent denominator is corrected to 95. The newer first-25 negative tranche stays separate from the earlier seven negatives until overlap is proven; the positive replay was revalidated, while the exact-ID route became unstable. Woohyuk q18 therefore remains OPEN / PARTIAL / ROUTE-UNSTABLE.
...
- Miner-local hesitation about Akariel does not override hard project canon: **Akariel = Zyrcant** remains resolved.

## Publication invariant

WIKI owns structured Cast dossiers, role progression, Relationships, Episodes, Running Gags, Quotes, Locations, and Gallery/media notes. MAIN owns richer biographies and lived character narrative. Shared evidence may appear on both surfaces only when each surface uses it for its own job; identical prose is not the goal.
