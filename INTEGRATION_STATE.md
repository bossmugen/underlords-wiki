...
### Published WIKI change — Chubi

Stable account `559670680287969281` remains **Chibiterasu / Chubi / Chuu~tan**. The Core tail adds a low-drama re-entry lane: 2022 self-presentation under `chibiterasu`, then a 2024 fresh welcome answered as `Chuu~tan`. WIKI now has the structured Chubi dossier and return/identity deepener; MAIN already owns the richer person-first version. The welcome sequence does not establish an exact departure, rejoin date, or appointment chronology.

Final accepted WIKI reader commit: **`d226d47d86e825399e89157ce1e7687503cb6b9f`**.

- **Build Underlords Wiki `35545778562`: succeeded.**
- **Deploy Underlords Wiki Preview / Pages `35545778580`: build succeeded and deployment succeeded.**

### Zoshaa / Nintendo — reconciliation without a duplicate owner

Hard canon remains **Zoshaa = NintendoShitcube / Nintendo / `nintendoshitcube_`**. The new Whiskey material deepens an already-public Zoshaa mechanism: old screenshots and `Throwback` language can make old room history current again with very little setup; the useful contradiction is offhand delivery with high social payload. Existing canonical WIKI Zoshaa already owns the old-screenshot / boiled-mayo material, so no duplicate Zoshaa reader paragraph was added.

A concurrent Run 1496 draft briefly tried to create `Nintendo` as a separate person. That draft was repaired and then superseded; final public state keeps Nintendo inside canonical Zoshaa. `POSTED BY` remains distinct from `MADE BY`, `CAPTURED BY`, and `FEATURING`.

### Same-name collisions held

Two concurrent proposed fusions were rejected because reviewed evidence anchors different stable accounts and there is no direct bridge:

- reviewed t7morrow handoff: `212165031293485056`; proposed same-name body: `249189376143802368`.
- reviewed Pride Wall handoff: `380825199836266497`; proposed same-name body: `387630572867928064`.

Same display name is not identity. Both remain held for explicit reconciliation.
...
Pride's Wall tail is duplicate/corroboration for an already-synthesized person mechanism. Queen Shimoko and Joy add bounded Wall texture but did not justify padding new standalone owners this run; Joy's `snow` / Wattpad referent remains open.
...
- Whiskey: consumed through `b3d73519…`; final result is reviewed/duplicate-corroboration plus identity reconciliation, not a separate Nintendo publication.
